...
export default function VismaEaccountingSettings() {
  ...
  // Queries
  const { data: settings, refetch: refetchSettings } = trpc.vismaEaccounting.getSettings.useQuery();
  const { data: statistics } = trpc.vismaEaccounting.getStatistics.useQuery();
  const { data: syncLogs } = trpc.vismaEaccounting.getSyncLogs.useQuery({ limit: 10 });
  const { data: lastSyncStatus } = trpc.vismaEaccounting.getLastSyncStatus.useQuery();
  const { data: customerMappings } = trpc.vismaEaccounting.getCustomerMappings.useQuery();
  const { data: invoiceMappings } = trpc.vismaEaccounting.getInvoiceMappings.useQuery();

  // Mutations
  const saveSettings = trpc.vismaEaccounting.saveSettings.useMutation({
    onSuccess: () => {
  ...
  const testConnection = trpc.vismaEaccounting.testConnection.useMutation({
    onSuccess: (data) => {
      if (data.success) {
  ...
  const toggleActive = trpc.vismaEaccounting.toggleActive.useMutation({
    onSuccess: () => {
  ...
  const syncAllCustomers = trpc.vismaEaccounting.syncAllCustomers.useMutation({
    onSuccess: (data) => {
  ...
  const performFullSync = trpc.vismaEaccounting.performFullSync.useMutation({
    onSuccess: (data) => {
      toast.success(
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Visma eAccounting Integrasjon</h1>
          <p className="text-muted-foreground">
            Administrer integrasjon med Visma eAccounting regnskapssystem
          </p>
        </div>
        {settings && (
      ...
      {settings && !settings.isActive && (
        <Alert>
          ...
          <AlertDescription>
            Visma eAccounting integrasjonen er deaktivert. Aktiver den for å starte synkronisering.
          </AlertDescription>
        </Alert>
      )}
        ...
        <TabsContent value="settings" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Konfigurer OAuth 2.0 tilkobling til Visma eAccounting
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Din Visma eAccounting Client ID"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Din Visma eAccounting Client Secret"
                />
              </div>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={testConnection.isLoading || !clientId || !clientSecret}
                >
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Manuell synkronisering av data til Visma eAccounting
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
        ...
        <TabsContent value="customers">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Oversikt over kunder synkronisert med Visma eAccounting
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                  ...
                  customerMappings.map((mapping) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Visma ID: {mapping.vismaCustomerId}
                        </p>
                      </div>
        ...
        <TabsContent value="invoices">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Oversikt over fakturaer synkronisert med Visma eAccounting
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                    ...
                    >
                      <div>
                        <p className="font-medium">
                          Faktura #{mapping.vismaInvoiceNumber || mapping.vismaInvoiceId}
                        </p>
                        <p className="text-sm text-muted-foreground">
        ...
        <TabsContent value="logs">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Historikk over synkroniseringer med Visma eAccounting
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
