...
export async function sendSMS(message: SMSMessage): Promise<SMSResult> {
  // Track SMS usage if tenant ID provided
  if (message.tenantId) {
    try {
      const { getDb } = await import("./db");
      const { tenants } = await import("../drizzle/schema");
      ...
      if (db) {
        ...
        if (tenant && tenant.smsPackageActive) {
          const smsSent = (tenant.smsSentThisMonth || 0) + 1;
          const smsLimit = tenant.smsPackageSize || 0;
          const overageRate = parseFloat(tenant.smsOverageRate || "1.00");
          
          // Calculate overage charge if over limit
          let overageCharge = parseFloat(tenant.smsOverageCharge || "0.00");
          if (smsSent > smsLimit) {
          ...
          await db
            ...
            .set({
              ...
              smsOverageCharge: overageCharge.toFixed(2),
            })
            .where(eq(tenants.id, message.tenantId));

          console.log(`[SMS] Tracked usage for tenant ${message.tenantId}: ${smsSent}/${smsLimit}`);
        }
      }
    } catch (error) {
      console.error("[SMS] Failed to track SMS usage:", error);
      // Continue sending SMS even if tracking fails
    }
  }
