...
export async function sendEmail(options: {
  ...
  subject: string;
  html: string;
  tenantId?: string; // Optional tenant ID for tracking
}): Promise<void> {
  ...
  // Track email usage if tenant ID provided
  if (options.tenantId) {
    try {
      ...
      if (db) {
        ...
        if (tenant) {
          const emailsSent = (tenant.emailsSentThisMonth || 0) + 1;
          const emailLimit = tenant.emailMonthlyLimit || 0;
          const overageRate = parseFloat(tenant.emailOverageRate || "0.10");
          
          // Calculate overage charge if over limit
          let overageCharge = parseFloat(tenant.emailOverageCharge || "0.00");
          ...
          await db
            ...
            .set({
              ...
              emailOverageCharge: overageCharge.toFixed(2),
            })
            .where(eq(tenants.id, options.tenantId));

          console.log(`[Email] Tracked usage for tenant ${options.tenantId}: ${emailsSent}/${emailLimit}`);
        }
      }
    } catch (error) {
      console.error("[Email] Failed to track email usage:", error);
