...
import { emailLimitsRouter } from "./server/emailLimits";
import { emailOverageBillingRouter } from "./server/emailOverageBilling";
import { smsPackagesRouter } from "./server/smsPackages";
...
export const appRouter = router({
  ...
  publicBooking: router({
    ...
    createBooking: publicProcedure
      .input(
        z.object({
          ...
          customerInfo: z.object({
            ...
            lastName: z.string().optional(),
            phone: z.string().regex(/^[0-9]{8,15}$/, "Phone number must be 8-15 digits"),
      ...
      .mutation(async ({ input }) => {
        ...
        const [appointment] = await dbInstance.insert(appointments).values({
          ...
          endTime,
          status: "pending",
          notes: input.customerInfo.notes || null,
        });
        ...
        if (input.customerInfo.email) {
          try {
            ...
            await sendEmail({
              ...
              subject: "Bekreftelse på din time",
              html: `
                <h2>Hei ${input.customerInfo.firstName}!</h2>
                <p>Din time er bekreftet.</p>
                <p><strong>Dato:</strong> ${input.date}</p>
                <p><strong>Tid:</strong> ${input.time}</p>
     ...
     * Allows customers to cancel their booking using booking ID and phone number
     */
    cancelBooking: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (bookingDateTime < now) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Cannot cancel past bookings" });
        }
        ...
        if (booking.customerEmail) {
          try {
            ...
            await sendEmail({
              ...
              subject: "Avbestilling bekreftet",
              html: `
                <h2>Hei ${booking.customerFirstName}!</h2>
                <p>Din time er avbestilt.</p>
                <p><strong>Dato:</strong> ${booking.appointmentDate}</p>
                <p><strong>Tid:</strong> ${booking.startTime}</p>
                <p>Velkommen tilbake en annen gang!</p>
              `,
            });
          } catch (emailError) {
    ...
    /**
     * Get booking details for modification
     * Allows customers to view their booking using booking ID and phone number
     */
    getBookingForModification: publicProcedure
      .input(z.object({
        bookingId: z.number().int().positive(),
        phone: z.string().min(8),
      }))
    ...
    /**
     * Modify a booking
     * Allows customers to change date/time of their booking
     */
    modifyBooking: publicProcedure
      .input(z.object({
        bookingId: z.number().int().positive(),
        phone: z.string().min(8),
        newDate: z.string(),
      ...
      .mutation(async ({ input }) => {
        ...
        if (booking.status === "canceled") {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Cannot modify canceled booking" });
        }

        if (booking.status === "completed") {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Cannot modify completed booking" });
        }
        ...
        const startMinutes = hours * 60 + minutes;
        const endMinutes = startMinutes + serviceDetails.durationMinutes;
        const endHour = Math.floor(endMinutes / 60);
        const endMinute = endMinutes % 60;
        ...
        // Send modification email if email is provided
        if (booking.customerEmail) {
          try {
            const { sendEmail } = await import("../email");
            await sendEmail({
              tenantId: booking.tenantId,
              to: booking.customerEmail,
              subject: "Endring av time bekreftet",
              html: `
                <h2>Hei ${booking.customerFirstName}!</h2>
                <p>Din time er endret.</p>
                <p><strong>Ny dato:</strong> ${input.newDate}</p>
                <p><strong>Ny tid:</strong> ${input.newTime}</p>
                <p>Vi gleder oss til å se deg!</p>
              `,
            });
          } catch (emailError) {
  ...
  // ============================================================================
  // EMAIL & SMS LIMITS AND BILLING
  // ============================================================================
  emailLimits: emailLimitsRouter,
  emailOverageBilling: emailOverageBillingRouter,
  smsPackages: smsPackagesRouter,
